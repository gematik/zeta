...
- OPA ist **verpflichtend** und kann nicht deaktiviert werden. Die früheren Toggles
  `opa.enabled`, `provider.smcB.opa.enabled` und `provider.smcB.failClosed` wurden
  entfernt; in bestehenden Override-Dateien werden sie nach dem Upgrade still ignoriert.
  Ist OPA nicht erreichbar, antwortet der Authserver mit `503 temporarily_unavailable`,
  statt ohne Policy-Entscheidung ein Token auszustellen.
- Zwei Betriebsarten:
  - Bundle‑Modus (Standard): Policy+Data werden als OCI‑Bundle aus einer Registry geladen.
  - Inline‑Policy: Rego‑Policy wird als ConfigMap gemountet (Alternative für einfache Demo/Tests).
- Entscheidungspfad: Aufrufer nutzen `POST /v1/data/policies/zeta/authz/decision` mit `{ "input": { ... } }` und erhalten `{ allow, ttl }` zurück.

## Aktivieren

- Bundle‑Modus (Chart-Standard seit Release 1.0.2): `zeta-guard.opa.bundle.enabled: true`
  (Registry + Auth konfigurieren)
- Inline‑Policy: `zeta-guard.opa.bundle.enabled: false`
