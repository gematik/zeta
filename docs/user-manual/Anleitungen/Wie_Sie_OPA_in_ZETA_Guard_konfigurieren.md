...
- Alternativ kann ein bereits vorhandener ServiceAccount genutzt werden:
  ...
  legt das Chart weder ServiceAccount noch RBAC selbst an — der angegebene
...
Der öffentliche Schlüssel für die Signaturprüfung wird **nicht** über Helm-Values gepflegt — er wird vom Provisioning-Daten-Image bereitgestellt
...
Ziel-Google-Service-Accounts (`sts.sa`, Impersonation) einen kurzlebigen
Access-Token ab. Dieser wird als Datei in ein Kubernetes-Secret geschrieben; OPA
liest ihn von dort.
...
- den Status-Updates von OPA, die den Bundle-Status samt Fehler enthalten. Diese
  gehen standardmäßig **nicht** ins Konsolen-Log (`opa.logStatusUpdates`,
  Standard `false`), sondern nur an das Telemetry-Gateway,
